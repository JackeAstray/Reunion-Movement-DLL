...
namespace ReunionMovementDLL.Dungeon.Shape
{
    ...
    /// 实现流程：采样房间中心 -> 放置房间 -> 计算最小生成树（Kruskal） -> 生成走廊 -> 可选添加额外边形成环路。
    ...
    public sealed class RogueLikeTinyKeep : RectBaseRogueLike<RogueLikeTinyKeep>, IDrawer<int>
    {
        ...
        /// <summary>
        /// 核心绘制逻辑：采样房间中心、放置房间、构建全连边并计算最小生成树（Kruskal），
        ...
        /// <summary>
        /// 并查集实现（用于 Kruskal MST），支持 Find 与 Union 操作。
        /// </summary>
        private class UnionFind
        {
            ...
            /// 查找元素所在集合的根索引（含路径压缩）。
            /// </summary>
            /// <param name="x">元素索引。</param>
            /// <returns>根索引。</returns>
            public int Find(int x) { return p[x] < 0 ? x : (p[x] = Find(p[x])); }

            /// <summary>
            /// 合并两个集合，若原已同属一集合返回 false，否则合并并返回 true。
            /// </summary>
            /// <param name="a">元素 a。</param>
            /// <param name="b">元素 b。</param>
            /// <returns>是否完成合并。</returns>
