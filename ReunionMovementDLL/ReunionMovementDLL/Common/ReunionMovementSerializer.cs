...
namespace ReunionMovementDLL
{
    ...
    public abstract class ReunionMovementSerializer<T>
    {
        ...
        /// 初始化游戏框架序列化器基类的新实例。
        /// </summary>
        public ReunionMovementSerializer()
        {
        ...
        public void RegisterSerializeCallback(byte version, SerializeCallback callback)
        {
            if (callback == null)
            {
                throw new ReunionMovementException("序列化回调无效。");
            }
        ...
        public void RegisterDeserializeCallback(byte version, DeserializeCallback callback)
        {
            if (callback == null)
            {
                throw new ReunionMovementException("反序列化回调无效。");
            }
        ...
        public void RegisterTryGetValueCallback(byte version, TryGetValueCallback callback)
        {
            if (callback == null)
            {
                throw new ReunionMovementException("尝试获取键值回调无效。");
            }
        ...
        public bool Serialize(Stream stream, T data)
        {
            ...
            {
                throw new ReunionMovementException("未注册序列化回调。");
            }
        ...
        public bool Serialize(Stream stream, T data, byte version)
        {
            ...
            {
                throw new ReunionMovementException(Utility.Text.Format("序列化回调 '{0}' 不存在。", version));
            }
        ...
        public T Deserialize(Stream stream)
        {
            ...
            {
                throw new ReunionMovementException(Utility.Text.Format("头标识无效，需要 '{0}{1}{2}'，当前 '{3}{4}{5}'。", (char)header[0], (char)header[1], (char)header[2], (char)header0, (char)header1, (char)header2));
            }
            ...
            {
                throw new ReunionMovementException(Utility.Text.Format("反序列化回调 '{0}' 不存在。", version));
            }
